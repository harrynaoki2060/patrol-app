...
var CACHE_NAME   = 'patrol-v10';      // ← バージョンを上げるだけで旧キャッシュが消える
    ...
    caches.open(CACHE_NAME)
      .then(function(cache) {
        ...
        return fetch('./index.html', { cache: 'reload' })
          ...
          .then(function() {
            console.log('[SW v10] ✓ index.html を 2 キーでキャッシュ完了');
          })
          // index.html のキャッシュ失敗は致命的だが install は続行する
          .catch(function(e) {
            console.error('[SW v10] ✗ index.html キャッシュ失敗:', e.message);
          })
            ...
            return Promise.all(
              extras.map(function(url) {
                return cache.add(url)
                  .then(function() { console.log('[SW v10] ✓ cached:', url); })
                  .catch(function(e) { console.warn('[SW v10] ✗ skip:', url, '-', e.message || e); });
              })
            );
          });
      })
      .then(function() {
        console.log('[SW v10] ▶ install 完了 → skipWaiting()');
        // 旧 SW をすぐ置き換える（waiting をスキップ）
        return self.skipWaiting();
      })
      .catch(function(e) {
        console.error('[SW v10] install エラー:', e);
...
self.addEventListener('activate', function(event) {
  console.log('[SW v10] ▶ activate 開始');
  ...
  event.waitUntil(
    ...
    caches.keys()
      .then(function(keys) {
        ...
        console.log('[SW v10] 削除対象キャッシュ:', targets);
        return Promise.all(
          targets.map(function(k) {
            return caches.delete(k)
              .then(function(ok) { console.log('[SW v10] ✓ 削除:', k, ok); })
              .catch(function(e) { console.warn('[SW v10] ✗ 削除失敗:', k, e); });
          })
        );
      })
      .then(function() {
        console.log('[SW v10] ▶ clients.claim()');
        // 開いているすべてのタブを即座に制御下に置く
        return self.clients.claim();
      })
      .then(function() {
        console.log('[SW v10] ✓ activate 完了 / キャッシュ名:', CACHE_NAME);
      })
      .catch(function(e) {
        console.error('[SW v10] activate エラー:', e);
...
self.addEventListener('message', function(event) {
  if (event.data && event.data.type === 'SKIP_WAITING') {
    console.log('[SW v10] message → SKIP_WAITING 受信 → skipWaiting()');
    self.skipWaiting();
  }
});
...
function handleNavigate(req) {
  return caches.open(CACHE_NAME).then(function(cache) {
    ...
    return cache.match(req)
      .then(function(hit) {
        if (hit) {
          console.log('[SW v10] navigate HIT(1) exact:', req.url);
        ...
        return cache.match('./index.html')
          .then(function(hit2) {
            if (hit2) {
              console.log('[SW v10] navigate HIT(2) ./index.html');
              return hit2;
            }

            // 【段3】ネットワーク取得 → キャッシュ更新
            console.log('[SW v10] navigate MISS → network fetch');
            return fetch(req, { cache: 'no-cache' })
              .then(function(res) {
                if (res && res.ok) {
                  ...
                  cache.put(req,            c1);
                  cache.put('./index.html', c2);
                  console.log('[SW v10] navigate network OK → 2キーキャッシュ更新');
                }
                return res;
              })
              .catch(function() {
                // 完全オフライン かつ キャッシュ空 → 内蔵フォールバック HTML
                console.warn('[SW v10] navigate OFFLINE → OFFLINE_PAGE 返却');
